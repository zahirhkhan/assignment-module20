        ...
        <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4"
                          class="mt-1 block w-full p-2 border border-gray-300 rounded-md">{{ old('description', $product->description) }}</textarea>
                @error('description')
                <p class="mt-2 text-red-600 text-sm">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="number" name="price" id="price" value="{{ old('price', $product->price) }}"
                       class="mt-1 block w-full p-2 border border-gray-300 rounded-md" step="0.01" required>
                @error('price')
                <p class="mt-2 text-red-600 text-sm">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="number" name="stock" id="stock" value="{{ old('stock', $product->stock) }}"
                       class="mt-1 block w-full p-2 border border-gray-300 rounded-md" min="0">
                @error('stock')
                <p class="mt-2 text-red-600 text-sm">{{ $message }}</p>
                @enderror
            </div>
